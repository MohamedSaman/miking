...
class StaffQuotationList extends Component
{
    ...
    {
        // Build base query filtered by staff's created_by
    ...
    public function viewQuotation($id)
    {
        ...
        if (!$quotation) {
            $this->dispatch('show-error', 'Quotation not found or access denied.');
            return;
        }
    ...
    public function openCreateSaleModal($quotationId)
    {
        ...
        if ($this->selectedQuotation) {
            ...
            $this->editableItems = collect($items)->map(function ($item) {
                ...
                // Get allocated quantity from StaffProduct table instead of admin stock
    ...
    public function searchProducts($searchTerm)
    {
        if (strlen($searchTerm) >= 2) {
            return \App\Models\StaffProduct::where('staff_id', Auth::id())
                ->join('product_details', 'staff_products.product_id', '=', 'product_details.id')
                ->leftJoin('product_prices', 'product_details.id', '=', 'product_prices.product_id')
                ->where(function ($query) use ($searchTerm) {
                    $query->where('product_details.name', 'like', '%' . $searchTerm . '%')
    ...
    {
        // Find product from staff's allocated products
        $staffProduct = \App\Models\StaffProduct::where('staff_id', Auth::id())
            ->where('product_id', $productId)
            ->first();

        if (!$staffProduct) {
            $this->dispatch('show-error', 'Product not found or not allocated to you.');
            return;
        }
        ...
        if ($product && isset($this->editableItems[$index])) {
            $currentStock = ($staffProduct->quantity - $staffProduct->sold_quantity);
    ...
    public function deleteQuotation()
    {
        try {
            ...
            $quotation = Quotation::where('id', $this->quotationToDelete)
                ->where('created_by', Auth::id())
                ->first();
    ...
    {
        // Build the paginated query filtered by staff's created_by
